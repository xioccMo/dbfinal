...
class TestChangeReceived:
    ...
    def pre_run_initialization(self):
        ...
        self.store_id = "test_change_unreceived_store_id_{}".format(str(uuid.uuid1()))
        self.buyer_id = "test_change_unreceived_buyer_id_{}".format(str(uuid.uuid1()))
    ...
    def test_buyer_id_error(self):
        code = change_received(self.buyer_id + "_x", self.buyer_password, self.order_id)
        assert code != 200

    def test_buyer_password_error(self):
        code = change_received(self.buyer_id, self.buyer_password + "_x",self.order_id)
        assert code != 200

    def test_order_id_error(self):
        code = change_received(self.buyer_id, self.buyer_password, self.order_id + "_x")
        assert code != 200
