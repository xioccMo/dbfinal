...
class TestSearchBookInSite:
    ...
    def test_status_search_ok(self):
        code = search_book_site(self.buyer_id, self.buyer_password, self.key_word)
        assert code == 200

    def test_error_user_id(self):
        code = search_book_site(self.buyer_id + "_x", self.buyer_password, self.key_word)
        assert code != 200

    def test_error_password_id(self):
        code = search_book_site(self.buyer_id, self.buyer_password + "_x", self.key_word)
        assert code != 200
